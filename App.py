...
    try:
        req = requests.Request("POST", url, auth=auth, json=payload)
        prepared = req.prepare()

        auth_header = prepared.headers.get("Authorization", "None")
        logger.info(f"OAuth1 Auth Header: {auth_header}")
...
    except Exception as e:
        logger.error(f"[{now}] [EXCEPTION] During tweet POST: {e}")
...
@app.route("/debug-twitter-auth")
def debug_auth():
    if not check_auth(request):
        return jsonify({"error": "Unauthorized"}), 401

    payload = {"text": "Testing OAuth1 debug"}
    try:
        req = requests.Request("POST", url, auth=auth, json=payload)
        prepared = req.prepare()
        return jsonify({
            "Authorization": prepared.headers.get("Authorization", "None"),
            "Endpoint": url
        })
    except Exception as e:
